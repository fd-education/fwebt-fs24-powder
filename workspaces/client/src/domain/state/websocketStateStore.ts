...
export const useWebsocketStore = create<WebsocketState>()((set, get) => ({
  ...
  emitGameChallenge: (req: ChallengeRequest) => {
    if (!get().isConnected) return;

    get().socket.emit(MultiplayerEvents.CHALLENGE, req);
  },
  ...
  registerChatHistoryHandler: (handler: (messages: ChatMessage[]) => void) => {
    ...
    get().socket.on(ChatEvents.CHAT_HISTORY, (messages: ChatMessage[]) =>
      handler(messages)
    );
  },
  registerChatHandler: (handler: (message: ChatMessage) => void) => {
    ...
    get().socket.on(ChatEvents.CHAT_MESSAGE, (message: ChatMessage) =>
      handler(message)
    );
  },
